...
class PDFMaker:
    ...
    # MAIN PDF CREATOR
    def generate_pdf_content(self, data: dict) -> bytes:
        buffer = BytesIO()
    ...
    def __build_title(self) -> list:
        return [
            Paragraph("AI-Generated Marketing Plan", self.styles["Heading1"]),
            Spacer(1, 16)
        ]

    def __build_campaign_overview(self, data: dict) -> list:
        elements = [
    ...
    def __build_execution_workflow(self, data: dict) -> list:
        elements = [
    ...
    def __build_channel_strategy(self, data: dict) -> list:
        elements = [
    ...
    def __build_channel_table(self, rows: list[dict], table_type:str="organic") -> list:
        if table_type == "organic":
            headers = ["Channel", "Content", "Frequency", "Example"]

            def extract(row: dict) -> list:
                return [
    ...
    def __build_conversion_strategy(self, data: dict) -> list:
    ...
    def __build_content_ideas(self, data: dict) -> list:
        elements = [
    ...
    # Table maker
    def __create_table(self, data: list, col_widths: list[int]) -> Table:
    ...
    @staticmethod
    def __clean_text(text: str) -> str:   # Helps deal with LLM garbage letters that are not ASCII and could potentially lead to errors
    ...
    @staticmethod
    def __add_footer(canvas: Canvas, doc) -> None:
